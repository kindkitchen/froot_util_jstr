...
import { is_json_non_primitive as test_target } from "./is_json_non_primitive.ts";
...
async function positive_case(t: Deno.TestContext) {
  await t.step("Should be a json non-primitive", async (t) => {
    for (
      const value of [
        {},
        [],
...
async function negative_case(t: Deno.TestContext) {
  await t.step("Should NOT be a json non-primitive", async (t) => {
    for (
      const value of [
        1,
        null,
